...
- [完整安装与使用手册](docs/INSTALLATION_AND_USAGE.md)
- [构建与发布](BUILD_AND_RELEASE.md)
- [备份、恢复与回滚](RESTORE_NOTES.md)
- [安全策略](SECURITY.md)
...
- 多 VPS SSH 节点目录、专用 Ed25519 密钥、公钥下发、批量探活和系统巡检
- 远程命令、验证密钥后关闭密码登录，以及新旧端口并存验证和失败回滚
...
这只适用于本地开发。生产环境请按[完整安装与使用手册](docs/INSTALLATION_AND_USAGE.md)部署 systemd、Nginx 和 HTTPS。

## 生产部署

仓库提供通用 `/opt/kunpanel` 布局的 `deploy/tryallfun-panel.service`、Nginx 模板和构建说明。面板默认只监听 `127.0.0.1:8088`，生产环境必须通过 HTTPS 反向代理访问，并限制管理入口来源。
